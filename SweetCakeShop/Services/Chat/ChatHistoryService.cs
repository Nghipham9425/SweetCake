...
namespace SweetCakeShop.Services.Chat
{
    public class ChatHistoryService : IChatHistoryService
    {
        ...
        public async Task<CustomerChatMessage> AddSystemMessageAsync(string content, CancellationToken ct = default)
        {
            var (userId, token) = _identity.GetIdentity();
            if (string.IsNullOrEmpty(userId) && string.IsNullOrEmpty(token))
                token = _identity.EnsureChatTokenCookie();
        ...
        public Task<bool> HasAnyMessagesAsync(CancellationToken ct = default) =>
            Query().Where(m => m.Sender != "system").AnyAsync(ct);

        public async Task<bool> IsHumanSupportActiveAsync(CancellationToken ct = default)
        {
            var marker = await Query()
                .Where(m => m.Sender == "system"
                    && (m.Content == "SUPPORT_OPEN" || m.Content == "SUPPORT_CLOSED"))
                .OrderByDescending(m => m.CreatedAt)
                .FirstOrDefaultAsync(ct);

            return marker?.Content == "SUPPORT_OPEN";
        }
